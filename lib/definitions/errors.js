...
export function EINVALIDTOKENENVVAR({ tokenEnvVar }) {
  return {
    message: "Invalid `tokenEnvVar` option.",
    details: `The [tokenEnvVar option](${linkify("README.md#options")}) option, if defined, must be a non-empty \`String\`.

Your configuration for the \`tokenEnvVar\` option is \`${tokenEnvVar}\`.`,
  };
}

export function EINVALIDALLOWSLOWTYPES({ allowSlowTypes }) {
  return {
    message: "Invalid `allowSlowTypes` option.",
    details: `The [allowSlowTypes option](${linkify("README.md#options")}) option, if defined, must be a \`Boolean\`.

Your configuration for the \`allowSlowTypes\` option is \`${allowSlowTypes}\`.`,
  };
}
